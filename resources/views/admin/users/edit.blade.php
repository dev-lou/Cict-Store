<x-admin-layout>
                            ...
                            <div class="flex items-center gap-6">
                                <div class="flex flex-col items-center">
                                    @if($user->profile_picture)
                                        <img id="profile-preview" src="{{ asset('storage/' . $user->profile_picture) }}?t={{ time() }}" alt="Profile" style="width: 120px; height: 120px; border-radius: 12px; object-fit: cover; border: 3px solid #b0bcc4; box-shadow: 0 4px 12px rgba(139, 0, 0, 0.2);">
                                    @else
    ...
    <script>
        function handleAdminProfilePictureUpload(input, userId) {
            ...
            // Upload file
            fetch('{{ route("admin.users.update-picture", ":id") }}'.replace(':id', userId), {
                method: 'POST',
                headers: {
