<x-admin-layout>
    ...
    <div class="p-6" style="background: var(--bg-main); min-height: calc(100vh - 4rem);">
        <div class="mb-6">
            <h1 class="text-3xl font-bold" style="color: var(--text-primary);">Add Service</h1>
            <p class="text-sm mt-1" style="color: var(--text-secondary);">Create a new student service</p>
        </div>
            ...
            <form action="{{ route('admin.services.store') }}" method="POST">
                ...
                <div class="form-grid">
                    <div class="full-width">
                        ...
                        <input type="hidden" name="icon" id="selectedIcon" value="{{ old('icon', '🖨️') }}">
                        <div class="icon-grid">
                            @foreach(['🖨️','🎨','📄','📦','🧾','✂️','🖼️','📐','💡','⚙️','🧵','🛠️','🎁','📊','🧮','🪄'] as $emoji)
                                <div class="icon-pick {{ old('icon', '🖨️') === $emoji ? 'selected' : '' }}" data-icon="{{ $emoji }}" onclick="selectIcon('{{ $emoji }}')">{{ $emoji }}</div>
                            @endforeach
                        </div>
                        @error('icon')
