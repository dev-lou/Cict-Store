<x-admin-layout>
    ...
    <div class="mb-8">
        ...
        <p class="mt-1" style="color: #b0bcc4;">View and manage {{ strtolower($pageTitle) }}</p>
    </div>
        ...
        <form method="GET" class="flex gap-4 flex-wrap items-end">
            ...
            <div>
                ...
                >
                    ...
                    <option value="today" {{ request('date_filter') == 'today' ? 'selected' : '' }} style="background-color: #0f1419; color: white;">Today</option>
                    <option value="3days" {{ request('date_filter') == '3days' ? 'selected' : '' }} style="background-color: #0f1419; color: white;">Last 3 Days</option>
                    <option value="week" {{ request('date_filter') == 'week' ? 'selected' : '' }} style="background-color: #0f1419; color: white;">Last Week</option>
                    <option value="month" {{ request('date_filter') == 'month' ? 'selected' : '' }} style="background-color: #0f1419; color: white;">Last Month</option>
                </select>
            </div>
            ...
            <!-- Clear Filters -->
            <a href="{{ request()->url() }}" class="px-6 py-2 rounded-lg font-bold transition-colors text-center" style="background-color: transparent; color: #b0bcc4; border: 2px solid #b0bcc4;" onmouseover="this.style.backgroundColor='#1a7fff'; this.style.color='white';" onmouseout="this.style.backgroundColor='transparent'; this.style.color='#b0bcc4';">
                Clear
            </a>
        </form>
    </div>
                ...
                <tbody>
                    @forelse($orders as $order)
                        <tr class="transition-colors" style="border-bottom: 2px solid #b0bcc4;" onmouseover="this.style.backgroundColor='#0f6fdd'" onmouseout="this.style.backgroundColor='transparent'">
                            <td class="px-6 py-4 font-bold" style="color: #b0bcc4;">#{{ $order->order_number }}</td>
                            <td class="px-6 py-4" style="color: #ffffff;">{{ $order->user->name ?? 'Unknown' }}</td>
                            <td class="px-6 py-4 font-bold" style="color: #ffffff;">₱{{ number_format($order->total, 2) }}</td>
                            <td class="px-6 py-4">
                                @php
