<x-admin-layout>
    ...
    <!-- Page Header -->
    <x-admin.page-header title="Order #{{ $order->order_number }}" subtitle="{{ $order->created_at->format('F d, Y h:i A') }}">
        <x-slot:actions>
            <div class="flex items-center gap-3">
                ...
                @endphp
                <span class="px-4 py-2 rounded-xl text-sm font-bold" style="{{ $statusStyles }}">{{ ucfirst($order->status) }}</span>
                    ...
                    <div class="space-y-4">
                        @forelse ($order->items as $item)
                            <div class="flex justify-between items-center py-4 px-4 rounded-lg" style="background: rgba(15, 111, 221, 0.05); border: 1px solid rgba(15, 111, 221, 0.2);">
                                <div>
                                    <p class="font-bold text-lg" style="color: #ffffff;">{{ optional($item->product)->name ?? $item->product_name ?? 'Product' }}</p>
                                    <p class="text-sm mt-1" style="color: #b0bcc4;">Quantity: <span class="font-semibold" style="color: #0f6fdd;">{{ $item->quantity }}</span></p>
                                </div>
                                <p class="font-bold text-xl" style="color: #0f6fdd;">₱{{ number_format($item->unit_price * $item->quantity, 2) }}</p>
                            </div>
                        @empty
                ...
                <div class="p-8">
                    ...
                    <div class="space-y-4">
                        <div class="flex justify-between items-center py-3 px-4 rounded-lg" style="background: rgba(15, 111, 221, 0.05); border: 1px solid rgba(15, 111, 221, 0.2);">
                            ...
                            <span style="color: #ffffff; font-weight: bold; font-size: 1.125rem;">₱{{ number_format($order->subtotal ?? $order->total, 2) }}</span>
                        </div>
                        <div class="flex justify-between items-center pt-4" style="border-top: 2px solid #444;">
                            ...
                            <span style="color: #0f6fdd; font-weight: bold; font-size: 1.5rem;">₱{{ number_format($order->total, 2) }}</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="p-8">
                    ...
                    <div class="space-y-4">
                        <div class="py-3 px-4 rounded-lg" style="background: rgba(15, 111, 221, 0.05); border: 1px solid rgba(15, 111, 221, 0.2);">
                            ...
                            <p style="color: #ffffff; font-weight: bold; font-size: 1.125rem;">{{ $order->user->name ?? 'Unknown' }}</p>
                        </div>
                        <div class="py-3 px-4 rounded-lg" style="background: rgba(15, 111, 221, 0.05); border: 1px solid rgba(15, 111, 221, 0.2);">
                            <p style="color: #b0bcc4; font-size: 0.75rem; margin-bottom: 4px;">Email</p>
                            <p style="color: #ffffff; font-weight: bold; font-size: 1.125rem;">{{ $order->user->email ?? 'N/A' }}</p>
                        </div>
                    </div>
                </div>
            </div>
