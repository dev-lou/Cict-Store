<x-admin-layout>
                    ...
                    @forelse($items as $item)
                        <tr data-id="{{ $item->id }}">
                            <td class="col-item-name">
                                <div class="cell-content" tabindex="0" onclick="editCell(this)" onkeydown="if(event.key==='Enter'){ editCell(this); }" data-field="item_name">{{ $item->item_name }}</div>
                            </td>
                            <td class="col-quantity">
                                <div class="cell-content" tabindex="0" onclick="editCell(this)" onkeydown="if(event.key==='Enter'){ editCell(this); }" data-field="quantity">{{ $item->quantity ?? 1 }}</div>
                            </td>
                            <td class="col-price">
                                <div class="cell-content" tabindex="0" onclick="editCell(this)" onkeydown="if(event.key==='Enter'){ editCell(this); }" data-field="price">
                                    @if($item->estimated_price_min && $item->estimated_price_max)
                                        <span class="price-text">₱{{ number_format($item->estimated_price_min, 2) }} - ₱{{ number_format($item->estimated_price_max, 2) }}</span>
                                    @else
                            ...
                            <td class="col-priority">
                                <div class="cell-content" tabindex="0" onclick="editCell(this)" onkeydown="if(event.key==='Enter'){ editCell(this); }" data-field="priority">
                                    <span class="priority-badge priority-{{ $item->priority }}">{{ ucfirst($item->priority) }}</span>
                                </div>
                            </td>
                            <td class="col-bought">
                                <div class="cell-content" tabindex="0" onclick="editCell(this)" onkeydown="if(event.key==='Enter'){ editCell(this); }" data-field="is_bought">
                                    <span class="status-badge bought-{{ $item->is_bought ? 'true' : 'false' }}">
                                        {{ $item->is_bought ? '✓ Yes' : '✗ No' }}
                                    </span>
                                </div>
                            </td>
                            <td class="col-notes">
                                <div class="cell-content" tabindex="0" onclick="editCell(this)" onkeydown="if(event.key==='Enter'){ editCell(this); }" data-field="notes">{{ $item->notes ?? '—' }}</div>
                            </td>
    ...
    <script>
        ...
        function updateItemField(itemId, field, value) {
            ...
            fetch(`{{ route('admin.buy-list.update', '') }}/${itemId}`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        function deleteItem(itemId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch(`{{ route('admin.buy-list.destroy', '') }}/${itemId}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'
                        }
