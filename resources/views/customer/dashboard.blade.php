        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                ...
                <div class="stat-card" style="background: linear-gradient(135deg, #DAA520, #B8860B);">
                    ...
                    <div class="stat-value text-lg">₱{{ number_format($totalSpent, 0) }}</div>
                </div>
            </div>
            ...
            <div class="dashboard-card mb-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <a href="{{ route('shop.index') }}" class="quick-action-btn">
                    ...
                    <a href="{{ route('account.orders') }}" class="quick-action-btn" style="background: linear-gradient(135deg, #eab308, #ca8a04);">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path>
                        </svg>
                        View Orders
                    ...
                    <a href="{{ route('notifications.index') }}" class="quick-action-btn" style="background: linear-gradient(135deg, #10b981, #059669); position: relative;">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path>
                        </svg>
                        Notifications
                        @if($unreadNotificationsCount > 0)
                            <span class="absolute -top-2 -right-2 bg-red-500 text-white text-xs font-bold px-2 py-1 rounded-full">{{ $unreadNotificationsCount }}</span>
                        @endif
                    </a>

                    <a href="{{ route('profile.edit') }}" class="quick-action-btn" style="background: linear-gradient(135deg, #6366f1, #4f46e5);">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="dashboard-card">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <a href="{{ route('account.orders') }}" class="text-sm font-semibold" style="color: #8B0000;">View All →</a>
                    </div>

                    @if($recentOrders->count() > 0)
                        <div class="space-y-0">
                            @foreach($recentOrders as $order)
                                <a href="{{ route('orders.show', $order) }}" class="order-item block">
                                    <div class="flex-1 min-w-0">
                                        <p class="font-bold text-sm" style="color: #1a1a1a;">{{ $order->order_number }}</p>
                                        <p class="text-xs text-gray-500 mt-1">{{ $order->created_at->format('M d, Y') }}</p>
                                    </div>
                                    <div class="text-right">
                                        <p class="font-bold" style="color: #8B0000;">₱{{ number_format($order->total, 2) }}</p>
                                        <span class="status-badge status-{{ $order->status }} mt-1">{{ ucfirst($order->status) }}</span>
                                    </div>
                                </a>
                            @endforeach
                        </div>
                    @else
                        <div class="text-center py-12">
                            ...
                            <a href="{{ route('shop.index') }}" class="inline-block mt-4 px-6 py-2 rounded-lg font-semibold" style="background: linear-gradient(135deg, #8B0000, #A00000); color: white;">Start Shopping</a>
                        </div>
                    @endif
                </div>
                ...
                <div class="dashboard-card">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <a href="{{ route('notifications.index') }}" class="text-sm font-semibold" style="color: #8B0000;">View All →</a>
                    </div>
