<x-app-layout :title="'Order Details - ' . config('app.name', 'CICT Dingle')">
    <style>
        :root {
            --ink: #0f172a;
            --muted: #475569;
    ...
    <div class="shell">
        <div class="order-header">
            ...
            <div style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
                ...
                <span
                    class="status {{ $order->status === 'completed' ? 'completed' : ($order->status === 'cancelled' ? 'cancelled' : 'pending') }}">{{ ucfirst($order->status) }}</span>
            </div>
        </div>
        ...
        <div class="grid">
                    ...
                    <div class="item">
                        <div class="thumb">
                            @if(optional($item->product)->image_path)
                                <img src="{{ optional($item->product)->image_url }}"
            ...
            <aside class="card">
                ...
                <div class="summary-row"><span>Subtotal</span><span
                        class="price">₱{{ number_format($order->subtotal, 2) }}</span></div>
                <div class="summary-total"><span>Total</span><span
                        class="price">₱{{ number_format($order->total, 2) }}</span></div>
            ...
            <div class="timeline">
                <div class="timeline-row">
                    ...
                    <div>
                        ...
                        <div class="price" style="color:var(--ink);">
                ...
                @if ($order->completed_at)
                    <div class="timeline-row">
                        ...
                        <div>
                            ...
                            <div class="price" style="color:var(--ink);">
                                {{ $order->completed_at->format('M d, Y \a\t g:i A') }}
                            </div>
                        </div>
                    </div>
                @else
