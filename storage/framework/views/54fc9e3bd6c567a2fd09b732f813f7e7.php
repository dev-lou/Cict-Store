...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.app-layout','data' => ['title' => 'Cart - ' . config('app.name', 'CICT Dingle')]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('Cart - ' . config('app.name', 'CICT Dingle'))]); ?>
    <style>
        :root {
            --ink: #0f172a;
            --muted: #475569;
    ...
    <div class="cart-shell">
        <div class="cart-hero">
            ...
            <div style="display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
                <span class="chip"><?php echo e($totalQty); ?> <?php echo e($totalQty === 1 ? 'item' : 'items'); ?></span>
                <a href="<?php echo e(route('shop.index')); ?>" class="pill-link">← Continue shopping</a>
            </div>
        </div>
            ...
            <div class="cart-grid">
                <section class="card">
                    <?php $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="item" data-key="<?php echo e($item['key']); ?>">
                            <div class="thumb">
                            ...
                            <div>
                                <div class="item-head">
                                    <h3><a href="<?php echo e(route('shop.show', $item['product'])); ?>" style="color:inherit; text-decoration:none;"><?php echo e($item['product']->name); ?></a></h3>
                                    <?php if($item['variant']): ?>
                                        <p>Variant: <?php echo e($item['variant']->name); ?></p>
                                    <?php endif; ?>
                                </div>
                                <div class="meta-row">
                                    <span class="subtle">₱<?php echo e(number_format($item['price'], 2)); ?> each</span>
                                    <div class="qty" aria-label="Quantity">
                                        <button type="button" onclick="changeQty('<?php echo e($item['key']); ?>', -1)">-</button>
                                        <input type="number" min="1" value="<?php echo e($item['quantity']); ?>" readonly class="qty-input" data-qty="<?php echo e($item['key']); ?>">
                                        <button type="button" onclick="changeQty('<?php echo e($item['key']); ?>', 1)">+</button>
                                    </div>
                                    <span class="price" data-subtotal="<?php echo e($item['key']); ?>">₱<?php echo e(number_format($item['total'], 2)); ?></span>
                                    <form id="remove-form-<?php echo e($item['key']); ?>" action="<?php echo e(route('cart.destroy', $item['key'])); ?>" method="POST" style="margin:0;">
                                        <?php echo csrf_field(); ?>
                                        <?php echo method_field('DELETE'); ?>
                                        <button type="button" class="remove" onclick="confirmRemove('<?php echo e($item['key']); ?>')">Remove</button>
                                    </form>
                                    <input type="hidden" class="price-value" data-price-key="<?php echo e($item['key']); ?>" value="<?php echo e($item['price']); ?>">
                                </div>
                            </div>
                        </div>
                ...
                <aside class="card pad summary">
                    <h3>Order summary</h3>
                    <div class="summary-row"><span>Subtotal</span><span class="price" data-summary-subtotal><?php echo e(number_format($subtotal, 2)); ?></span></div>
                    <div class="summary-total"><span>Total</span><span class="price" data-summary-total><?php echo e(number_format($total, 2)); ?></span></div>

                    <?php if(auth()->guard()->check()): ?>
                        <button class="primary-btn" onclick="window.location='<?php echo e(route('checkout.index')); ?>'">Proceed to checkout</button>
                    <?php else: ?>
                        <a href="<?php echo e(route('login')); ?>" class="primary-btn" style="display:inline-flex; justify-content:center; text-decoration:none;">Sign in to checkout</a>
                    <?php endif; ?>

                    <button class="ghost-btn" style="margin-top:10px;" onclick="window.location='<?php echo e(route('shop.index')); ?>'">Add more items</button>
        ...
        <?php else: ?>
            <div class="card pad empty">
                ...
                <a class="primary-btn" style="display:inline-block; text-decoration:none; width:auto; padding-inline:18px;" href="<?php echo e(route('shop.index')); ?>">Start shopping</a>
            </div>
        <?php endif; ?>
    </div>
